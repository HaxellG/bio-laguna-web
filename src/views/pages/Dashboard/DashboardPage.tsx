        ...
        {!selectedDeviceId ? (
          ...
          <>
            {readings.length > 0 && preset === 'last_available' && (
              <div className="flex items-center gap-3 bg-white border border-gray-100 shadow-sm px-4 py-2 rounded-xl w-fit mb-3">
                <div className="flex items-center gap-1.5 text-xs font-medium text-gray-500">
                  <span className="material-icons-round text-[16px] text-primary-500">history</span>
                  Últimos 60 registros
                </div>
                <div className="w-px h-4 bg-gray-200"></div>
                <div className="flex items-center gap-2 text-xs font-semibold text-gray-700">
                  <span className="bg-gray-50 px-2 py-1 rounded-lg border border-gray-200">
                    {format(readings[0].timestamp, 'dd/MM/yy HH:mm')}
                  </span>
                  <span className="material-icons-round text-[14px] text-gray-400">arrow_forward</span>
                  <span className="bg-primary-50 text-primary-700 px-2 py-1 rounded-lg border border-primary-100">
                    {format(readings[readings.length - 1].timestamp, 'dd/MM/yy HH:mm')}
                  </span>
                </div>
              </div>
            )}
